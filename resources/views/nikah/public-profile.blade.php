<x-guest-layout :title="__('db.:age years, :city — Sallaamti Nikah', ['age' => $profile->age, 'city' => $profile->city])" :description="__('db.Verified Nikah profile on Sallaamti: :age years old, :city, :status. Sign up to view the full profile.', ['age' => $profile->age, 'city' => $profile->city, 'status' => ucfirst(str_replace('_', ' ', $profile->marital_status))])">
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <dl class="text-sm space-y-2">
                    @if ($profile->height)
                    <div class="flex justify-between"><dt class="text-gray-500">{{ __('db.Height') }}</dt><dd>{{ $profile->height }}</dd></div>
                    @endif
                    @if ($profile->education)
                    <div class="flex justify-between"><dt class="text-gray-500">{{ __('db.Education') }}</dt><dd>{{ $profile->education }}</dd></div>
                    @endif
                    @if ($profile->profession)
                    <div class="flex justify-between"><dt class="text-gray-500">{{ __('db.Profession') }}</dt><dd>{{ $profile->profession }}</dd></div>
                    @endif
                    @if ($profile->family_type)
                    <div class="flex justify-between"><dt class="text-gray-500">{{ __('db.Family Type') }}</dt><dd>{{ $profile->family_type }}</dd></div>
                    @endif
                </dl>

                @if ($profile->about)
                <div class="mt-4 pt-4 border-t">
                    <h5 class="font-semibold text-gray-700 mb-2 text-sm">{{ __('db.About') }}</h5>
            ...
            <div class="rounded-xl p-6 text-center" style="background: linear-gradient(135deg, #0d6b6b 0%, #1a1a2e 100%);">
                <p class="text-white font-semibold mb-3">💍 {{ __('db.View the full profile, photos & show your interest') }}</p>
                <a href="{{ route('nikah.profile.view', $profile) }}" class="inline-block bg-[--gold] text-white text-sm font-semibold px-6 py-3 rounded-lg" style="background:#b8962e">
                    {{ __('db.View Full Profile') }} →
                </a>
            </div>
            @else
            <div class="rounded-xl p-6 text-center" style="background: linear-gradient(135deg, #0d6b6b 0%, #1a1a2e 100%);">
                <p class="text-white font-semibold mb-3">🔒 {{ __('db.Sign up on Sallaamti to see the full profile, photos & express interest') }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 text-center">
                <h5 class="font-semibold text-gray-700 mb-3 text-sm">{{ __('db.Share this profile') }}</h5>
                <div class="flex gap-2 justify-center flex-wrap">
                    <a href="https://wa.me/?text={{ urlencode(__('db.Check out this profile on Sallaamti: ') . url()->current()) }}" target="_blank"
                        class="inline-flex items-center gap-2 bg-green-500 text-white text-sm px-4 py-2 rounded-lg hover:bg-green-600">
                        <i class="fab fa-whatsapp"></i> {{ __('db.WhatsApp') }}
                    </a>
                    <button type="button" onclick="sallaamtiShareProfile()"
                        ...
                        <i class="fa fa-share-alt"></i> {{ __('db.Share') }}
                    </button>
    ...
    <script>
        function sallaamtiShareProfile() {
            ...
            if (navigator.share) {
                navigator.share({ title: {!! json_encode(__('db.Sallaamti Nikah Profile')) !!}, text: {!! json_encode(__('db.Check out this profile on Sallaamti')) !!}, url });
            } else {
        ...
        function sallaamtiCopyProfileLink(btn) {
            navigator.clipboard.writeText(window.location.href).then(() => {
                if (btn) {
                    ...
                    btn.innerHTML = '✅ ' + {!! json_encode(__('db.Copied!')) !!};
