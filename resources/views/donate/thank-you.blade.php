<x-guest-layout>
    <section class="min-h-screen flex items-center justify-center py-20 bg-cream">
        <div class="max-w-lg mx-auto px-4 text-center">
            ...
            <h1 class="text-3xl font-extrabold text-gray-800 mb-3">{{ __('db.JazakAllah Khair!') }}</h1>
            <p class="text-gray-500 mb-2">{{ __('db.Your donation has been submitted successfully.') }}</p>
            <p class="text-gray-500 mb-8">{!! __('db.Our team will verify and confirm within :hours, in sha Allah.', ['hours' => '<strong>' . __('db.24 hours') . '</strong>']) !!}</p>
            ...
            <div class="rounded-2xl p-6 mb-8" style="background: linear-gradient(135deg, #0d6b6b 0%, #1a1a2e 100%)">
                <p class="text-white/90 italic text-base leading-relaxed mb-2">
                    {{ __('db."The example of those who spend their wealth in the way of Allah is like a seed of grain which grows seven spikes; in each spike is a hundred grains."') }}
                </p>
                <p class="text-white/50 text-sm">{{ __('db.Surah Al-Baqarah 2:261') }}</p>
            </div>
            ...
            <div class="bg-white rounded-2xl p-5 shadow-sm mb-6 text-sm text-gray-600">
                <p class="flex items-center gap-2 justify-center mb-1">
                    ...
                    {{ __('db.A confirmation email has been sent to your inbox.') }}
                </p>
                <p class="text-gray-400 text-xs">{{ __("db.Check your spam folder if you don't see it within a few minutes.") }}</p>
            </div>
