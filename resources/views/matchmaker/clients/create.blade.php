<x-matchmaker-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm">
            ...
            <span class="text-gray-700 font-semibold">{{ __('db.Add Client') }}</span>
        </div>
    </x-slot>

    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h2 class="text-lg font-bold text-gray-800 mb-1">{{ __('db.Add a New Client') }}</h2>
            <p class="text-sm text-gray-500 mb-6">{{ __("db.They'll be assigned to you automatically.") }}</p>
            ...
            <form method="POST" action="{{ route('matchmaker.clients.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <x-input-label for="looking_for" :value="__('db.Looking For')" />
                        <select id="looking_for" name="looking_for" class="border-gray-300 rounded-md shadow-sm w-full mt-1">
                            <option value="">{{ __('db.Unknown') }}</option>
                            <option value="self" {{ old('looking_for') === 'self' ? 'selected' : '' }}>{{ __('db.Self') }}</option>
                            <option value="family_member" {{ old('looking_for') === 'family_member' ? 'selected' : '' }}>{{ __('db.Family Member') }}</option>
                        </select>
                    </div>
                    <div class="col-span-2">
                    ...
                    @if ($canManageTeam)
                    <div class="col-span-2">
                        <x-input-label for="assigned_to" :value="__('db.Assign To')" />
                        <select id="assigned_to" name="assigned_to" class="border-gray-300 rounded-md shadow-sm w-full mt-1">
                            <option value="">{{ __('db.Me (:name)', ['name' => auth()->user()->name]) }}</option>
                    ...
                    @endif
                    <div class="col-span-2">
                        <x-input-label for="next_follow_up_at" :value="__('db.Next Follow-up (optional)')" />
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <a href="{{ route('matchmaker.clients.index') }}" class="text-sm text-gray-500 px-4 py-2.5 hover:underline">{{ __('db.Cancel') }}</a>
                    <button class="text-white text-sm font-semibold px-5 py-2.5 rounded-lg hover:opacity-90 hover:-translate-y-0.5 transition shadow-sm" style="background: var(--mm-plum);">{{ __('db.Add Client') }}</button>
                </div>
            </form>
        </div>
    </div>
