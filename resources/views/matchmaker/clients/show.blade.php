                ...
                <form method="POST" action="{{ route('matchmaker.clients.update', $lead) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="border border-gray-200 bg-gray-50 rounded-md shadow-sm w-full mt-1 px-3 py-2 text-sm text-gray-700">
                            {{ $lead->createdBy?->name ?? __('db.Unknown') }}
                        </div>
                        <p class="text-xs text-gray-400 mt-1">{{ __("db.Locked — this lead was created by this counselor, so it can't be reassigned to anyone else.") }}</p>
                        @else
