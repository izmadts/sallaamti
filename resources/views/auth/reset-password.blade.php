    ...
    <div class="auth-wrapper">
        <div class="auth-card">
            ...
            <div class="text-center mb-8">
                ...
                <h2 class="text-2xl font-extrabold text-gray-800">{{ __('db.Set New Password') }}</h2>
                <p class="text-gray-500 text-sm mt-2">{{ __('db.Choose a strong new password for your account.') }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.store') }}" class="space-y-5">
                ...
                <div>
                    <label class="auth-label">{{ __('db.New Password') }}</label>
                ...
                <div>
                    <label class="auth-label">{{ __('db.Confirm New Password') }}</label>
                    <div class="auth-input-wrap" x-data="{ show: false }">
                        <i class="fa fa-lock auth-icon"></i>
                        <input :type="show ? 'text' : 'password'" name="password_confirmation" required
                            class="auth-input @error('password_confirmation') auth-input-error @enderror"
                            placeholder="{{ __('db.Repeat new password') }}">
                        <button type="button" @click="show = !show" class="auth-eye-btn">
                            <i :class="show ? 'fa fa-eye-slash' : 'fa fa-eye'"></i>
                        </button>
                    </div>
                ...
                <button type="submit" class="btn-base btn-teal w-full py-3 text-base font-semibold">
                    {{ __('db.Reset Password') }} <i class="fa fa-check ml-2"></i>
                </button>
            </form>
