                ...
                <form method="POST" action="{{ route('quran-live.admission.step.save', [$course, 'preferences']) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="teacher_preference" class="border-gray-300 rounded-md w-full mt-1" required>
                            <option value="no_preference" {{ $currentTeacherPref === 'no_preference' ? 'selected' : '' }}>No Preference</option>
                            <option value="male" {{ $currentTeacherPref === 'male' ? 'selected' : '' }}>Male Teacher</option>
                            <option value="female" {{ $currentTeacherPref === 'female' ? 'selected' : '' }}>Female Teacher</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="class_type" class="border-gray-300 rounded-md w-full mt-1">
                            <option value="group" {{ $currentClassType === 'group' ? 'selected' : '' }}>Group Class</option>
                            <option value="one_to_one" {{ $currentClassType === 'one_to_one' ? 'selected' : '' }}>One-to-One (Private)</option>
                        </select>
                    </div>
                    <div>
                        <x-input-label value="Special Requirements / Comments (optional)" />
                        <textarea name="comments" rows="3" class="border-gray-300 rounded-md w-full mt-1">{{ old('comments', $data['comments'] ?? '') }}</textarea>
                    </div>

                    <div class="flex justify-between pt-2">
                        <a href="{{ route('quran-live.admission.step', [$course, 'student']) }}" class="btn-base text-gray-600 border border-gray-300 px-4 py-2 rounded-md hover:bg-gray-50">← Back</a>
