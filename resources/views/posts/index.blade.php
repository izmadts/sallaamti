<x-guest-layout title="Community Posts & Stories" description="Real stories, reflections, and announcements shared by the Sallaamti community.">
    ...
    <section class="page-hero relative overflow-hidden flex items-center" style="min-height: 220px; background: linear-gradient(135deg, #0d6b6b 0%, #1a1a2e 100%);">
        <div class="max-w-4xl mx-auto px-4 py-14 relative z-10 text-center w-full">
            <span class="section-eyebrow" style="color:#e0c26b;">Community Stories</span>
            <h1 class="text-3xl md:text-4xl font-extrabold text-white mt-2 mb-3">Posts from the Sallaamti Community</h1>
            <p class="text-white/70 text-sm max-w-xl mx-auto">Stories, reflections, and announcements shared by our members — reviewed by our team before going live.</p>
            @auth
            <a href="{{ route('posts.create') }}" class="inline-block mt-5 bg-white text-teal-800 font-semibold text-sm px-5 py-2.5 rounded-lg hover:opacity-90 transition">✍️ Share Your Story</a>
            @else
            <a href="{{ route('login') }}" class="inline-block mt-5 bg-white text-teal-800 font-semibold text-sm px-5 py-2.5 rounded-lg hover:opacity-90 transition">Log In to Share a Post</a>
            @endauth
        </div>
    </section>
    ...
    <section class="py-14 bg-gray-50">
        <div class="max-w-5xl mx-auto px-4">
            @if ($posts->isEmpty())
            <p class="text-center text-gray-500 py-10">No posts yet — be the first to share something with the community.</p>
            @else
