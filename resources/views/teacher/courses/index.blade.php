        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white rounded-lg shadow-sm p-5 text-center border-l-4 border-teal-500">
                    ...
                    <div class="text-sm text-gray-500 mt-1">My Groups</div>
                </div>
                ...
                <a href="{{ route('teacher.students.index') }}" class="bg-white rounded-lg shadow-sm p-5 text-center border-l-4 border-green-500 hover:shadow-md">
                    <div class="text-3xl font-bold">{{ $groups->sum(fn($g) => $g->activeStudents->count()) }}</div>
                    <div class="text-sm text-gray-500 mt-1">All Students</div>
                </a>
                <a href="{{ route('teacher.schedule') }}" class="bg-white rounded-lg shadow-sm p-5 text-center border-l-4 border-purple-500 hover:shadow-md">
                    ...
                    <div class="text-sm text-gray-500 mt-1">My Schedule</div>
                </a>
            </div>

            {{-- Today's Classes --}}
            <div class="bg-white rounded-lg shadow-sm p-6">
                <h3 class="font-semibold text-gray-700 mb-3">Today's Classes ({{ now()->format('l, d M') }})</h3>
                ...
                <div class="flex justify-between items-center border rounded-lg p-4 mb-2">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ $group->class_time }} | {{ $group->activeStudents->count() }} students</p>
                    </div>
                    @if ($group->todaysLink())
                    <span class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">✅ Link posted</span>
                    @else
                ...
                @empty
                <p class="text-gray-400 text-sm">No classes scheduled for today.</p>
                @endforelse
            </div>

            {{-- All Groups --}}
            <div class="bg-white rounded-lg shadow-sm p-6">
                <h3 class="font-semibold text-gray-700 mb-3">All My Groups</h3>
                <div class="space-y-3">
                    ...
                    <a href="{{ route('teacher.groups.show', $group) }}"
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">{{ $group->activeStudents->count() }} active students</p>
                        </div>
                        <div class="text-right">
                            @if ($group->todaysLink())
                            <span class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full block mb-1">✅ Link posted</span>
                            @else
                            <span class="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded-full block mb-1">No link yet</span>
                            @endif
                            <span class="text-xs text-teal-600">Open →</span>
                        </div>
                    </a>
                    @empty
                    <p class="text-gray-400 text-sm">No groups assigned yet.</p>
                    @endforelse
                </div>
            </div>
