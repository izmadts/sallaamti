<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">My Students</h2>
    </x-slot>
    <div class="py-12">
            ...
            <div class="bg-white rounded-lg shadow-sm divide-y">
                ...
                <a href="{{ route('teacher.students.show', $gs) }}" class="flex justify-between items-center p-4 hover:bg-gray-50">
                    <div>
                        ...
                        @if ($gs->admission)
                        <p class="text-xs text-gray-400">{{ $gs->admission->student_name }} | Age: {{ $gs->admission->student_age }}</p>
                        @endif
                    </div>
                    <span class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">Active</span>
                </a>
                @empty
                <p class="p-5 text-gray-500">No students yet.</p>
                @endforelse
            </div>
            {{ $students->links() }}
        </div>
