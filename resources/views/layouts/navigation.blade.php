<nav x-data="{ open: false }" class="bg-yellow-500 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex items-center">
                    ...
                    <x-nav-link :href="route('courses.index')" :active="request()->routeIs('courses.*') || request()->routeIs('lessons.*') || request()->routeIs('quiz.*') || request()->routeIs('lesson.quiz.*')">
                        📖 Quran
                    </x-nav-link>
                    ...
                    <x-nav-link
                        :href="auth()->user()->nikahProfile ? route('nikah.show') : route('nikah.create')"
                        :active="request()->routeIs('nikah.*')">
                        💍 Nikah
                    ...
                    <x-nav-link :href="route('donations.create')" :active="request()->routeIs('donate.*') || request()->routeIs('donations.*')">
                        💝 Donate
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-2">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-2 py-2 border border-transparent text-sm font-medium rounded-md text-gray-500 hover:text-gray-700 focus:outline-none transition">
                            {{ Auth::user()->name }}
            ...
            <div class="-me-2 flex items-center sm:hidden gap-2">
                <!-- Mobile Bell -->
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="relative p-2 text-gray-500">
                        🔔
                        @if (Auth::user()->unreadNotifications->count() > 0)
                        <span class="absolute top-0 right-0 bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
                            {{ Auth::user()->unreadNotifications->count() }}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('donations.create')">💝 Donate</x-responsive-nav-link>
            @role('admin')
