<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">New Group — {{ $course->title }}</h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 bg-white rounded-lg shadow-sm p-6">
            <form method="POST" action="{{ route('admin.quran-live-courses.groups.store', $course) }}" class="space-y-4">
                @csrf
                <div>
                    <x-input-label value="Group Name" />
                    <x-text-input name="group_name" class="w-full mt-1" placeholder="e.g. Group A - Morning (Males)" required />
                </div>
                <div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-2 mt-1 text-sm">
                        ...
                        <label class="flex items-center gap-1">
                            <input type="checkbox" name="class_days[]" value="{{ $day }}"> {{ $day }}
                        </label>
                        @endforeach
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input name="class_time" class="w-full mt-1" placeholder="6:00 PM - 6:45 PM" required />
                    </div>
                    <div>
