    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow-sm rounded-lg p-4 text-sm text-gray-600">
                <p><strong>Reporter:</strong> {{ $report->reporter?->user?->name ?? 'Deleted profile' }}</p>
                <p><strong>Reported:</strong> {{ $report->reported?->user?->name ?? 'Deleted profile' }}</p>
            ...
            <div class="bg-white shadow-sm rounded-lg p-4 space-y-3">
                ...
                <div class="p-3 rounded {{ $message->sender_id === $report->reporter?->user_id ? 'bg-blue-50' : 'bg-gray-50' }}">
                    <div class="flex justify-between text-xs text-gray-400 mb-1">
                        <span class="font-semibold text-gray-600">{{ $message->sender?->name ?? 'Deleted account' }}</span>
