    ...
    <div class="max-w-3xl space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm divide-y">
            ...
            <form method="POST" action="{{ route('admin.languages.update', $language) }}" class="p-4 flex flex-wrap gap-3 items-center">
                ...
                <input type="text" name="name" value="{{ $language->name }}" class="border-gray-300 rounded text-sm flex-1 min-w-[120px]">
        ...
        <div class="bg-white rounded-lg shadow-sm p-5">
            ...
            <form method="POST" action="{{ route('admin.languages.store') }}" class="flex flex-wrap gap-3 items-end">
                ...
                <div>
                    ...
                    <input type="text" name="language" placeholder="e.g. ar" required maxlength="10" class="border-gray-300 rounded text-sm w-28">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">Display Name</label>
                    <input type="text" name="name" placeholder="e.g. العربية" required maxlength="50" class="border-gray-300 rounded text-sm w-48">
                </div>
                <label class="flex items-center gap-1 text-xs text-gray-600 pb-2">
                    <input type="checkbox" name="is_rtl" value="1"> RTL
                </label>
                <button class="bg-teal-700 text-white text-sm px-4 py-2 rounded hover:bg-teal-800">+ Add Language</button>
            </form>
        </div>
